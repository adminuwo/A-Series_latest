...
export default function AiHire() {
    ...
    const handleResumeFiles = async (files) => {
        ...
        for (const file of Array.from(files)) {
            ...
            if (!allowed.includes(file.type) && !file.name.match(/\.(pdf|doc|docx|txt)$/i)) continue;
            ...
            if (isDocx) {
                try {
                    ...
                    setHireCandidateProfiles(prev => prev
                        ? `${prev}\n---\n📄 ${file.name}: [Failed to extract — paste manually]`
                        : `📄 ${file.name}: [Failed to extract — paste manually]`);
                    setHireUploadedFiles(prev => [...prev, file.name]);
                }
            } else if (isTxt) {
                ...
                setHireCandidateProfiles(prev => prev
                    ? `${prev}\n---\n📄 ${file.name}:\n[Resume uploaded — AI will reference this file: ${file.name}]`
                    : `📄 ${file.name}:\n[Resume uploaded — AI will reference this file: ${file.name}]`);
                setHireUploadedFiles(prev => [...prev, file.name]);
            }
        }
    ...
    const handleAction = async (e, customPrompt = null) => {
        ...
        try {
            ...
            ` : hiringMode === 'Offer' ? `
            GOAL: Closing strategy. Salary: ₹${hireOfferSalary}. Equity: ${hireEquityPercent}%. Competitor: ₹${hireCompetitorSalary}. Perks: ${hireOfferPerks}. Leverage: ${hireCandidateLeverage}.
            SECTIONS: MARKET BENCHMARK, EQUITY ANALYSIS, ACCEPTANCE PROBABILITY, NEGOTIATION PLAYBOOK, VISUALIZATIONS.
            ` : hiringMode === 'Planning' ? `
            GOAL: Org design. Structure: ${hireOrgStructure}. Values: ${hireCulturalValues}.
            SECTIONS: SCALABILITY AUDIT, CAPACITY PLANNING, REPORTING LINES, SUCCESSION PLAN, VISUALIZATIONS.
            ` : `
            GOAL: Talent Analytics. Focus on funnel efficiency and cost-per-hire metrics.
            ...
            const response = await generateChatResponse([...messages, userMsg], finalInput, systemInstruction, [], 'English', null, null, { agentType: 'AIHIRE' });
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="bg-white rounded-[40px] p-8 border border-border/40 shadow-xl shadow-emerald-500/5 space-y-6">
                                            <div className="flex items-center justify-between">
                                                <div className="flex items-center gap-4">
                                                    <div className="p-3 bg-emerald-50 text-emerald-600 rounded-2-xl">
                                                    ...
                                                    <div className="flex flex-wrap gap-2 mt-4">
                                                        {hireUploadedFiles.map(f => (
                                                            <div key={f} className="flex items-center gap-2 px-3 py-1.5 bg-white border border-border/40 rounded-full text-[10px] font-bold text-maintext">
                                                                ...
                                                                {f}
                                                                <button onClick={() => setHireUploadedFiles(prev => prev.filter(x => x !== f))} className="hover:text-red-500"><X className="w-3 h-3" /></button>
                                                            </div>
                                                        ))}
                                                    </div>
                                                )}
