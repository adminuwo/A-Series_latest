...
const AIBIZInputs = ({
    ...
    const tabs = [
        ...
        { id: 'Automation', icon: Zap, label: 'Logic' },
    ];

    return (
        <div className="w-full space-y-8 animate-in fade-in slide-in-from-bottom-5 duration-1000">
            {/* SaaS Command Header */}
            <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6 bg-white/40 backdrop-blur-xl border border-white/60 p-5 lg:p-6 rounded-[32px] shadow-sm">
                <div className="flex items-center gap-4 lg:gap-5 w-full lg:w-auto">
                    <div className="w-12 h-12 lg:w-14 lg:h-14 shrink-0 rounded-2xl bg-red-600 flex items-center justify-center text-white shadow-xl shadow-red-500/20">
                        <Rocket size={24} className="lg:w-7 lg:h-7" />
                    </div>
                    <div className="min-w-0">
                        <h1 className="text-xl lg:text-2xl font-black text-slate-800 tracking-tight truncate">SaaS Intel <span className="text-red-600">OS</span></h1>
                        <div className="flex items-center gap-2 mt-1">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"></span>
                            <span className="text-[9px] lg:text-[10px] font-bold text-slate-500 uppercase tracking-widest truncate">System Operational</span>
                        </div>
                    </div>
                </div>

                <div className="flex items-center gap-2 bg-slate-100/50 p-1.5 rounded-2xl border border-slate-200/50 w-full lg:w-auto overflow-x-auto no-scrollbar scrollbar-none">
                    <div className="flex items-center gap-2 shrink-0">
                        {tabs.map((tab) => (
                            <button
                                key={tab.id}
                                onClick={() => setAibizMode(tab.id)}
                                className={`flex items-center gap-2 px-4 lg:px-5 py-2 lg:py-2.5 rounded-xl text-[10px] lg:text-[11px] font-black uppercase tracking-wider transition-all whitespace-nowrap ${currentMode === tab.id
                                    ? 'bg-white text-red-600 shadow-md ring-1 ring-slate-200'
                                    : 'text-slate-400 hover:text-slate-600 hover:bg-white/40'
                                    }`}
                            >
                                <tab.icon size={12} className="lg:w-3.5 lg:h-3.5" />
                                {tab.label}
                            </button>
                        ))}
                    </div>
            ...
            <div className="grid grid-cols-12 gap-8">
                ...
                <div className="bg-white rounded-[32px] lg:rounded-[45px] border border-slate-100 shadow-xl shadow-slate-200/20 min-h-[500px] flex flex-col overflow-hidden">
 
                        {/* Area Charts / Dashboard Content */}
                        <AnimatePresence mode="wait">
                            {currentMode === 'Dashboard' && (
                                <motion.div key="dashboard" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="p-5 lg:p-10 space-y-8 lg:space-y-10">
                                    {/* Top Row: Health Score & Growth Insights */}
                                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8">
                                        {/* Business Health Score */}
                                        <div className="bg-white border border-slate-100 rounded-[32px] lg:rounded-[40px] p-6 lg:p-8 shadow-sm relative overflow-hidden group">
                                            <div className="absolute -top-10 -right-10 opacity-5 group-hover:scale-110 transition-transform">
                                                <Activity size={160} className="text-red-600" />
                                            </div>
                                            <div className="relative z-10 w-full">
                                                <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-6">Business Health</h3>
                                                <div className="flex flex-col xs:flex-row items-center gap-6 lg:gap-8">
                                                    <div className="relative w-24 h-24 lg:w-28 lg:h-28 flex items-center justify-center shrink-0">
                                                        ...
                                                        <span className="absolute text-xl lg:text-2xl font-black text-slate-800 tabular-nums">{healthScore.score}%</span>
                                                    </div>
                                                    <div>
                                                        <p className={`text-[10px] lg:text-[11px] font-black uppercase mb-1 ${healthScore.status === 'Stable' ? 'text-emerald-600' : 'text-amber-600'}`}>Status: {healthScore.status}</p>
                                                        <div className="space-y-1.5">
                                                            {healthScore.weakAreas.map((area, i) => (
                                                                <div key={i} className="flex items-center gap-2 text-[8px] lg:text-[9px] font-bold text-slate-500">
                                        ...
                                        {/* Growth Insights */}
                                        <div className="lg:col-span-2 bg-slate-900 rounded-[32px] lg:rounded-[40px] p-6 lg:p-8 text-white relative overflow-hidden shadow-2xl group">
                                                    ...
                                                    <div className="space-y-3">
                                                        <div className="p-3 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors">
                                                            <p className="text-[10px] font-medium leading-relaxed italic text-slate-300">
                                                                "Conversion improved **12%** this week."
                                                            </p>
                                                        </div>
                                                        <div className="p-3 bg-white/5 rounded-2xl border border-white/10 hover:bg-white/10 transition-colors">
                                                            <p className="text-[10px] font-medium leading-relaxed italic text-slate-300">
                                                                "Cold leads increased by **18%**."
                                                            </p>
                                                        </div>
                                                    </div>
                                        ...
                                        <div className="grid grid-cols-1 gap-4">
                                            {customers.map((customer) => (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex flex-col xs:flex-row items-start xs:items-center gap-4 lg:gap-8 text-left xs:text-right w-full xs:w-auto mt-4 xs:mt-0 pt-4 xs:pt-0 border-t xs:border-t-0 border-slate-50">
                                                        <div>
                                                            <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Health Score</p>
                                                            <div className="flex items-center justify-start xs:justify-end gap-2">
                                                        ...
                                                        <div className="w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:text-red-500 group-hover:bg-red-50 transition-all self-end xs:self-center">
                                                            <ArrowRight size={20} />
                                                        </div>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div key={lead.id} className="p-6 bg-white border border-slate-100 rounded-[30px] hover:shadow-xl transition-all group overflow-hidden relative">
                                                        ...
                                                        <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between relative z-10 gap-6 lg:gap-0">
                                                            <div className="flex items-center gap-4 lg:gap-6">
                                                                <div className="w-12 h-12 lg:w-14 lg:h-14 shrink-0 rounded-2xl bg-slate-900 flex items-center justify-center text-white text-lg font-black">
                                                                    {lead.name.charAt(0)}
                                                                </div>
                                                                <div>
                                                                    <h4 className="font-black text-slate-800 tracking-tight text-base lg:text-lg">{lead.name}</h4>
                                                                    <div className="flex items-center gap-3 mt-1">
                                                                        <span className="text-[9px] lg:text-[10px] font-bold text-slate-400 uppercase tracking-widest">{lead.industry}</span>
                                                                        <span className="w-1 h-1 rounded-full bg-slate-200"></span>
                                                                        <span className="text-[9px] lg:text-[10px] font-bold text-red-600 uppercase">Budget: {lead.budget}</span>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-6 lg:gap-10 w-full lg:w-auto pt-4 lg:pt-0 border-t lg:border-t-0 border-slate-50">
                                                                <div className="grid grid-cols-3 gap-4 lg:gap-6 text-center flex-1 lg:flex-none w-full sm:w-auto">
                                                                    <div>
                                                                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1">Freq</p>
                                                                ...
                                                                <div className="text-left sm:text-right lg:border-l lg:border-slate-100 lg:pl-8 flex lg:block items-center justify-between w-full lg:w-auto">
                                                     ...
                                                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6">
                                                        ...
                                                        <div>
                                                            ...
                                                            <select
                                                                ...
                                                                className="w-full bg-slate-50 border border-slate-200 rounded-2xl p-3 text-[10px] font-bold outline-none focus:border-red-500 transition-all cursor-pointer"
                                                            >
